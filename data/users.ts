...
export async function getUserByID(userid: number): Promise<any> {
	let [result]: any = await pool.query("SELECT users.user_id, users.creation_date, users.first_name, users.last_name, users.birth_date, settings.model FROM users INNER JOIN settings ON settings.user_id = users.user_id WHERE users.user_id = ? AND users.deleted = 0", [userid]);
	return result[0];
}
