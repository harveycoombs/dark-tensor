...
export async function createUser(email: string, password: string, firstName: string, lastName: string): Promise<string> {
    ...
    const result = await pool.query("INSERT INTO users (user_id, creation_date, email_address, password, first_name, last_name) VALUES (gen_random_uuid(), NOW(), $1, $2, $3, $4) RETURNING user_id", [email, passwordHash, firstName, lastName]);
...
export async function updateUser(userid: string, firstName: string, lastName: string, occupation: string, emailAddress: string): Promise<boolean> {
    const result = await pool.query("UPDATE users SET first_name = $1, last_name = $2, occupation = $3, email_address = $4 WHERE user_id = $5", [firstName, lastName, occupation, emailAddress, userid]);
...
export async function updateSettings(userid: string, theme: string, model: string, visionModel: string, style: string): Promise<boolean> {
    const result = await pool.query("UPDATE settings SET theme = $1, model = $2, vision_model = $3, style = $4 WHERE user_id = $5", [theme, model, visionModel, style, userid]);
    return (result.rowCount ?? 0) > 0;
}
...
export async function getUserByID(userid: string): Promise<any> {
    const result = await pool.query("SELECT users.user_id, creation_date, first_name, last_name, settings.model, settings.vision_model FROM users INNER JOIN settings ON settings.user_id = users.user_id WHERE users.user_id = $1 AND deleted = false", [userid]);
    return result.rows[0];
}

export async function getUserDetails(userid: string): Promise<any> {
    const result = await pool.query("SELECT user_id, creation_date, email_address, first_name, last_name, occupation FROM users WHERE user_id = $1 AND deleted = false", [userid]);
    return result.rows[0];
}
...
export async function getUserByEmailAddress(email: string): Promise<any> {
    const result = await pool.query("SELECT user_id, first_name, last_name FROM users WHERE email_address = $1 AND deleted = false", [email]);
    return result.rows[0];
}

export async function getPasswordHash(email: string): Promise<string> {
    const result = await pool.query("SELECT password AS password FROM users WHERE email_address = $1", [email]);
