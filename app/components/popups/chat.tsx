        ...
        <Popup title="Chat" onClose={onClose}>
            <div className={`w-1300 flex${conversationHistoryIsVisible ? " gap-3" :""}`}>
                <div className="w-full pt-2.5">
                    ...
                    {loading ? <div className="px-3 py-0.5 mt-4 text-lg max-w-23/50 rounded-lg bg-blue-400 text-white w-fit mb-5"><FontAwesomeIcon icon={faEllipsis} className="animate-pulse" /></div> : null}
                    </div>
...
function ChatMessage({ message, ...rest }: any) {
    return (
        <div className={`w-5/12 mt-4 ${message.you ? "ml-auto" : "mr-auto"}`}>
            <div className={`px-3 py-2 text-sm max-w-23/50 rounded-lg ${message.you ? "bg-gray-100 text-gray-400" : "bg-blue-400 text-white"}`} {...rest}>{message.content.split("\n").map((line: string, index: number) => <span key={index}>{line}<br/></span>)}</div>
