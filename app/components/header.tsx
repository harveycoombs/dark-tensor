        ...
        <header className="p-3 select-none bg-white sticky top-0 border-b border-b-slate-300 flex justify-between items-center z-30">
            ...
            <div>{
                    user ? <>
                        ...
                        <div className="inline-grid align-middle place-items-center bg-sky-100 text-sky-500 text-[0.8rem] leading-none select-none font-medium w-8 h-8 rounded-full">{(user.first_name.charAt(0).toUpperCase() + user.last_name.charAt(0)).toUpperCase()}</div>
                    </> : <>
            ...
            {chatPopupIsVisible && user ? <ChatPopup model={user.model} onClose={() => setChatPopupVisibility(false)} /> : null}
        </header>
    );
}
