...
export default function Popup({ title, children, classes, onClose, ...rest }: Properties) {
    return (
        <div id="popup" className="fixed inset-0 w-screen h-screen z-50 bg-slate-950/60 grid place-items-center" onMouseDown={(e: any) => {if (e.target.matches("#popup")) onClose() }}>
            <motion.div initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1, transition: { duration: 0.2, ease: "easeOut" }}} className={`py-3 min-w-96 bg-white rounded-md ${classes?.length ? " " + classes : ""}`} {...rest}>
                    <div className="px-3 pb-3 mb-3 flex justify-between items-center leading-none border-b border-b-slate-300">
