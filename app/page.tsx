        ...
        (async () => {
            ...
            setRecentSearchesArea(<div className="mt-12">
                <h3 className="font-medium text-slate-400 mb-2">Recent Searches</h3>
                {
                    ...
                    : <div className="grid grid-cols-4 gap-4">{json.searches.map((search: any, index: any) => <Tile key={index} icon={faMagnifyingGlass} classes="flex flex-col justify-between" onClick={performRecentSearch}><p>{search.query}</p><div className="text-xs font-medium text-slate-400/60 mt-1.5">{new Date(search.search_date).toLocaleString(undefined, { month: "short", day: "numeric", hour: "numeric", minute: "numeric" })}</div></Tile>)}</div>
                }
            </div>);
        })();
        ...
        <>
            ...
            <main className="h-[calc(100vh-110px)] grid place-items-center">
                <motion.div className="w-650 mx-auto max-[700px]:w-full max-[700px]:px-3" initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: "easeOut" }}>
                    <div className="w-fit mx-auto" title="Collate AI"><Logo width={328} height={74} /></div>
